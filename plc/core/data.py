...
class DimmerGroup:
    ...
    def get_dimmers_at(self, level):
        ...
        for obj, intensity in self.nested.items():
            for d, i in obj.get_dimmers_at(intensity):
...
class Group(DimmerGroup):
    ...
    def keep_zeros(self, val):
        self.__discard_zeros = not val

    def __getitem__(self, i):
        return self.channels[i]

    def __setitem__(self, i, val):
        if self.__discard_zero and i == 0:
            del self.channels[i]
        else:
            _verify_level(i)
            self.channels[i] = val
